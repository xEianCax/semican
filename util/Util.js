...
class Util {
	...
	static async verify(channel, user, time = 30000) {
		const filter = res => {
			...
			return res.author.id === user.id && (evthaci.includes(value) || hyrhaci.includes(value));
		};
		...
		if (evthaci.includes(choice)) return true;
		if (hyrhaci.includes(choice)) return false;
		return false;
	}
}
